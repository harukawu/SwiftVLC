#!/usr/bin/env bash
#
# verify.sh — Build the DynamicHost fixture app (iOS simulator) and assert
# the supported layered-app topology yields exactly ONE dynamic libVLC
# framework image among the images the app loads: libvlc.framework/libvlc.
# The app executable and Swift wrapper frameworks must not define libVLC
# symbols themselves.
#
...
audit_app() { # $1 = .app path
  ...
  if [[ "${#defining[@]}" -eq 1 && "$exe_defines" -eq 0 &&
    "${defining[0]}" == */libvlc.framework/libvlc ]]; then
    echo "PASS: exactly one libVLC dynamic framework image"
  else
    echo "FAIL: expected exactly one libVLC defining image, libvlc.framework/libvlc;" \
